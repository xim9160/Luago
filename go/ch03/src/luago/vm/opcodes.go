...
const (
	...
	OP_LOADNIL;			OP_GETUPVAL; OP_GETABUP; 	OP_GETTABLE;
...
const (
	OpArgN = iota 	// argument is not used 表示不会被使用, 比如 MOVE指令, 只使用了 A B 不使用 C
	OpArgU 			// argument is used 
	OpArgR			// argument is a register or a jump offset IABC 模式下表示索引, IAsBx 中表示偏移, 以 MOVE 指令为例子, 表示将 B 的值移动到 A 中 : R(A) := R(B)
	OpArgK			// argument is a constant or register/constant 常量表索引 或者 寄存器索引
)
...
var opcodes = []opcode {
	...
	opcode{0,	1,	OpArgR, OpArgR, IABC, "GETTABLE"},
	...
	opcode{0,   0,  OpArgR, OpArgN, IAsBx,"JUMP    "},
